...
struct TRTCRemoteStatistics {
  ...
  /// 【字段含义】音频流的总丢包率（％）。
  ///  audioPacketLoss
  ///  代表音频流历经`主播>云端>观众`这样一条完整的传输链路后，最终在观众端统计到的丢包率。
  ...
  /// 【字段含义】该路视频流的总丢包率（％）。
  ///  videoPacketLoss
  ///  代表该路视频流历经`主播>云端>观众`这样一条完整的传输链路后，最终在观众端统计到的丢包率。
  ...
  ///  的延迟，更准确地说，它代表了“采集=>编码=>网络传输=>接收=>缓冲=>解码=>播放” 全链路的延迟。
  ///  point2PointDelay 需要本地和远端的 SDK 均为 8.5 及以上的版本才生效，若远端用户为 8.5
  ///  以前的版本，此数值会一直为0，代表无意义。
  ...
  /// 【字段含义】音频播放卡顿率，单位 (%)
  /// 音频播放卡顿率（audioBlockRate） = 音频播放的累计卡顿时长（audioTotalBlockTime） /
  /// 音频播放的总时长
  ...
  /// 【字段含义】该路音视频流的总丢包率（％）。已废弃，不推荐使用；建议使用
  /// audioPacketLoss、videoPacketLoss 替代。
  uint32_t finalLoss;

  /// 【字段含义】该用户从 SDK 到云端的上行丢包率，单位 (%)
  /// 该数值越小越好，如果 remoteNetworkUplinkLoss 为
  /// 0%，则意味着上行链路的网络质量很好，上传到云端的数据包基本不发生丢失。 如果
  /// remoteNetworkUplinkLoss 为 30%，则意味着 SDK 向云端发送的音视频数据包中，会有 30%
  /// 丢失在传输链路中。
  uint32_t remoteNetworkUplinkLoss;

  /// 【字段含义】该用户从 SDK 到云端的往返延时，单位 ms
  /// 该数值代表从 SDK 发送一个网络包到云端，再从云端回送一个网络包到 SDK
  /// 的总计耗时，也就是一个网络包经历 “SDK=>云端=>SDK” 的总耗时。 该数值越小越好：如果
  /// remoteNetworkRTT < 50ms，意味着较低的音视频通话延迟；如果 remoteNetworkRTT >
  /// 200ms，则意味着较高的音视频通话延迟。 需要特别解释的是，remoteNetworkRTT 代表 “SDK=>云端=>SDK”
  /// 的总耗时，所以不需要区分 remoteNetworkUpRTT 和 remoteNetworkDownRTT
...
struct TRTCStatistics {
  ...
  /// 该数值越小越好，如果 upLoss 为
  /// 0%，则意味着上行链路的网络质量很好，上传到云端的数据包基本不发生丢失。 如果 upLoss 为
  /// 30%，则意味着 SDK 向云端发送的音视频数据包中，会有 30% 丢失在传输链路中。
  uint32_t upLoss;

  /// 【字段含义】从云端到 SDK 的下行丢包率，单位 (%)
  /// 该数值越小越好，如果 downLoss 为
  /// 0%，则意味着下行链路的网络质量很好，从云端接收的数据包基本不发生丢失。 如果 downLoss 为
  /// 30%，则意味着云端向 SDK 传输的音视频数据包中，会有 30% 丢失在传输链路中。
  uint32_t downLoss;

  /// 【字段含义】从 SDK 到云端的往返延时，单位 ms
  /// 该数值代表从 SDK 发送一个网络包到云端，再从云端回送一个网络包到 SDK
  /// 的总计耗时，也就是一个网络包经历 “SDK=>云端=>SDK” 的总耗时。 该数值越小越好：如果 rtt <
  /// 50ms，意味着较低的音视频通话延迟；如果 rtt > 200ms，则意味着较高的音视频通话延迟。
  /// 需要特别解释的是，rtt 代表 “SDK=>云端=>SDK” 的总耗时，所以不需要区分 upRtt 和 downRtt。
  uint32_t rtt;

  /// 【字段含义】从 SDK 到本地路由器的往返时延，单位 ms 该数值代表从 SDK
  /// 发送一个网络包到本地路由器网关，再从网关回送一个网络包到 SDK 的总计耗时，也就是一个网络包经历
  /// 【SDK>网关>SDK【 的总耗时。 该数值越小越好：如果 gatewayRtt <
  /// 50ms，意味着较低的音视频通话延迟；如果 gatewayRtt > 200ms，则意味着较高的音视频通话延迟。
  /// 当网络类型为蜂窝网时，该值无效。
  uint32_t gatewayRtt;
