...
class ITRTCCloudCallback {
 public:
   ...
   * 2.1 进入房间成功与否的事件回调
   *
   * 调用 TRTCCloud 中的 enterRoom() 接口执行进房操作后，会收到来自 ITRTCCloudCallback 的
   * onEnterRoom(result) 回调：
   * - 如果加入成功，回调 result 会是一个正数（result >
   * 0），代表进入房间所消耗的时间，单位是毫秒（ms）。
   ...
   * @note
   * 1. 在 Ver6.6 之前的版本，只有进房成功会抛出 onEnterRoom(result) 回调，进房失败由 onError()
   * 回调抛出。
   * 2. 在 Ver6.6 之后的版本：无论进房成功或失败，均会抛出 onEnterRoom(result)
   * 回调，同时进房失败也会有 onError() 回调抛出。
   */
   ...
   * 接口会执行退出房间的相关逻辑，例如释放音视频设备资源和编解码器资源等。 待 SDK
   * 占用的所有资源释放完毕后，SDK 会抛出 onExitRoom() 回调通知到您。 如果您要再次调用 enterRoom
   * 或者切换到其他的音视频 SDK，请等待 onExitRoom
   * 回调到来后再执行相关操作。否则可能会遇到例如摄像头、麦克风设备被强占等各种异常问题。
   ...
   * 2.3 切换角色的事件回调
   *
   * 调用 TRTCCloud 中的 switchRole() 接口会切换主播和观众的角色，该操作会伴随一个线路切换的过程，待
   * SDK 切换完成后，会抛出 onSwitchRole() 事件回调。
   * @param errCode 错误码，ERR_NULL
   * 代表切换成功，其他请参见[错误码](https://cloud.tencent.com/document/product/647/32257)。
   * @param errMsg  错误信息。
   */
   ...
   * 2.4 切换房间的结果回调
   *
   * 调用 TRTCCloud 中的 switchRoom() 接口可以让用户快速地从一个房间切换到另一个房间，待 SDK
   * 切换完成后，会抛出 onSwitchRoom() 事件回调。
   * @param errCode 错误码，ERR_NULL
   * 代表切换成功，其他请参见[错误码](https://cloud.tencent.com/document/product/647/32257)。
   * @param errMsg  错误信息。
   */
   ...
   * 接口会将两个不同房间中的主播拉通视频通话，也就是所谓的“主播PK”功能。 调用者会收到
   * onConnectOtherRoom() 回调来获知跨房通话是否成功，
   * 如果成功，两个房间中的所有用户都会收到来自另一个房间中的 PK 主播的 onUserVideoAvailable()
   * 回调。
   ...
   * 3.1 有用户加入当前房间
   *
   * 出于性能方面的考虑，在 TRTC 两种不同的应用场景（即 AppScene，在 enterRoom
   * 时通过第二个参数指定）下，该通知的行为会有差别：
   * - 直播类场景（TRTCAppSceneLIVE 和
   * TRTCAppSceneVoiceChatRoom）：该场景下的用户区分主播和观众两种角色，只有主播进入房间时才会触发该通知，观众进入房间时不会触发该通知。
   * - 通话类场景（TRTCAppSceneVideoCall 和
   * TRTCAppSceneAudioCall）：该场景下的用户没有角色的区分（可认为都是主播），任何用户进入房间都会触发该通知。
   * @param userId 远端用户的用户标识。
   * @note
   * 1. 事件回调 onRemoteUserEnterRoom 和 onRemoteUserLeaveRoom
   * 只适用于维护当前房间里的“用户列表”，有此事件回调不代表一定有视频画面。
   ...
   * 3.2 有用户离开当前房间
   *
   * 与 onRemoteUserEnterRoom 相对应，在两种不同的应用场景（即 AppScene，在 enterRoom
   * 时通过第二个参数指定）下，该通知的行为会有差别：
   * - 直播类场景（TRTCAppSceneLIVE 和
   * TRTCAppSceneVoiceChatRoom）：只有主播离开房间时才会触发该通知，观众离开房间不会触发该通知。
   * - 通话类场景（TRTCAppSceneVideoCall 和
   * TRTCAppSceneAudioCall）：该场景下用户没有角色的区别，任何用户的离开都会触发该通知。
   * @param userId 远端用户的用户标识。
   * @param reason
   ...
   *
   * **主路画面** 一般被用于承载摄像头画面。当您收到 onUserVideoAvailable(userId, true)
   * 通知时，表示该路画面已经有可播放的视频帧到达。 此时，您需要调用 {@link startRemoteView}
   * 接口订阅该用户的远程画面，订阅成功后，您会继续收到该用户的首帧画面渲染回调
   * onFirstVideoFrame(userid)。 当您收到 onUserVideoAvailable(userId, false)
   * 通知时，表示该路远程画面已经被关闭，关闭的原因可能是该用户调用了 {@link muteLocalVideo} 或
   * {@link stopLocalPreview}。
   * @param userId 远端用户的用户标识。
   ...
   *
   * “辅路画面”一般被用于承载屏幕分享的画面。当您收到 onUserSubStreamAvailable(userId, true)
   * 通知时，表示该路画面已经有可播放的视频帧到达。 此时，您需要调用 {@link startRemoteView}
   * 接口订阅该用户的远程画面，订阅成功后，您会继续收到该用户的首帧画面渲染回调
   * onFirstVideoFrame(userid)。
   * @param userId 远端用户的用户标识。
   * @param available 该用户是否发布（或取消发布）了辅路视频画面，true: 发布；false：取消发布。
   * @note 显示辅路画面使用的函数是 {@link startRemoteView} 而非 {@link
   * startRemoteSubStreamView}，startRemoteSubStreamView 已经被废弃。
   */
   ...
   *
   * 当您收到 onUserAudioAvailable(userId, true) 通知时，表示该用户发布了自己的声音，此时 SDK
   * 的表现为：
   ...
   * @note SDK 默认使用自动订阅模式，您可以通过 {@link setDefaultStreamRecvMode}
   * 设置为手动订阅，但需要在您进入房间之前调用才生效。
   */
   ...
   *
   * SDK 会在渲染自己本地或远端用户的首帧画面时抛出该事件，您可以通过回调事件中的 userId
   * 参数来判断事件来自于“本地”还是来自于“远端”。
   * - 如果 userId 为空值，代表 SDK 已经开始渲染自己本地的视频画面，不过前提是您已经调用了 {@link
   * startLocalPreview} 或 {@link startScreenCapture}。
   * - 如果 userId 不为空，代表 SDK 已经开始渲染远端用户的视频画面，不过前提是您已经调用了 {@link
   * startRemoteView} 订阅了该用户的视频画面。
   * @param userId 本地或远端的用户标识，如果 userId 为空值代表自己本地的首帧画面已到来，userId
   * 不为空则代表远端用户的首帧画面已到来。
   * @param streamType
   ...
   * @param height 画面的高度。
   * @note
   * 1. 只有当您调用了 {@link startLocalPreview} 或 {@link startScreenCapture}
   * 之后，才会触发自己本地的首帧画面事件回调。
   * 2. 只有当您调用了 {@link startRemoteView} 或 {@link startRemoteSubStreamView}
   * 之后，才会触发远端用户的首帧画面事件回调。
   */
  virtual void onFirstVideoFrame(const char* userId,
   ...
   *
   * 当您成功进入房间并通过 {@link startLocalPreview} 或 {@link startScreenCapture}
   * 开启本地视频采集之后（开启采集和进入房间的先后顺序无影响），SDK
   * 就会开始进行视频编码，并通过自身的网络模块向云端发布自己本地的视频数据。当 SDK
   ...
   *
   * 当您成功进入房间并通过 {@link startLocalAudio}
   * 开启本地音频采集之后（开启采集和进入房间的先后顺序无影响），SDK
   * 就会开始进行音频编码，并通过自身的网络模块向云端发布自己本地的音频数据。当 SDK
   ...
   * 3.10 远端视频状态变化的事件回调
   *
   * 您可以通过此事件回调获取远端每一路画面的播放状态（包括 Playing、Loading 和 Stopped
   * 三种状态），从而进行相应的 UI 展示。
   * @param userId 用户标识。
   * @param streamType
   * 视频流类型：主路（Main）一般用于承载摄像头画面，辅路（Sub）一般用于承载屏幕分享画面。
   ...
   */
  virtual void onRemoteVideoStatusUpdated(const char* userId,
   ...
   * 3.11 远端音频状态变化的事件回调
   *
   * 您可以通过此事件回调获取远端每一路音频的播放状态（包括 Playing、Loading 和 Stopped
   * 三种状态），从而进行相应的 UI 展示。
   * @param userId 用户标识。
   ...
   * @param extrainfo 额外信息。
   */
  virtual void onRemoteAudioStatusUpdated(const char* userId,
                                          TRTCAVStatusType status,
                                          TRTCAVStatusChangeReason reason,
                                          void* extrainfo) {}
   ...
   * 通知时，表示该路画面大小发生了调整，调整的原因可能是该用户调用了 setVideoEncoderParam 或者
   * setSubStreamEncoderParam 重新设置了画面尺寸。
   * @param userId 用户标识。
   * @param streamType
   * 视频流类型：主路（Main）一般用于承载摄像头画面，辅路（Sub）一般用于承载屏幕分享画面。
   ...
   */
  virtual void onUserVideoSizeChanged(const char* userId,
                                      TRTCVideoStreamType streamType,
                                      int newWidth,
                                      int newHeight) {}
   ...
   * SDK
   * 会使用一组内嵌的自研算法对当前网络的延迟高低、带宽大小以及稳定情况进行评估，并计算出一个的评估结果：
   * 如果评估结果为 1（Excellent） 代表当前的网络情况非常好，如果评估结果为
   * 6（Down）代表当前网络无法支撑 TRTC 的正常通话。
   ...
   * 4.2 音视频技术指标的实时统计回调
   *
   * 该统计回调每间隔2秒抛出一次，用于通知 SDK 内部音频、视频以及网络相关的专业技术指标，这些信息在
   * {@link TRTCStatistics} 均有罗列：
   ...
   * @param statistics 统计数据，包括自己本地的统计信息和远端用户的统计信息，详情请参考 {@link
   * TRTCStatistics}。
   * @note 如果您只需要获知当前网络质量的好坏，并不需要花太多时间研究本统计回调，更推荐您使用 {@link
   * onNetworkQuality} 来解决问题。
   */
   ...
   * @param result 网速测试数据数据，包括丢包、往返延迟、上下行的带宽速率，详情请参见 {@link
   * TRTCSpeedTestResult}。
   */
   ...
   * 在抛出此事件之后，SDK 会努力跟云端重新建立连接，重连过程中会抛出 {@link
   * onTryToReconnect}，连接恢复后会抛出 {@link onConnectionRecovery} 。 所以，SDK
   * 会在如下三个连接相关的事件中按如下规律切换：
   ...
   * 5.2 SDK 正在尝试重新连接到云端
   *
   * SDK 会在跟云端的连接断开时抛出 {@link
   * onConnectionLost}，之后会努力跟云端重新建立连接并抛出本事件，连接恢复后会抛出 {@link
   ...
   * 5.3 SDK 与云端的连接已经恢复
   *
   * SDK 会在跟云端的连接断开时抛出 {@link onConnectionLost}，之后会努力跟云端重新建立连接并抛出
   * {@link onTryToReconnect}，连接恢复后会抛出本事件回调。
   */
   ...
   * 6.1 摄像头准备就绪
   *
   * 当您调用 {@link startLocalPreivew} 之后，SDK
   * 会尝试启动摄像头，如果摄像头能够启动成功就会抛出本事件。
   ...
   * 6.2 麦克风准备就绪
   *
   * 当您调用 {@link startLocalAudio} 之后，SDK
   * 会尝试启动麦克风，如果麦克风能够启动成功就会抛出本事件。
   ...
   * 6.4 音量大小的反馈回调
   *
   * SDK 可以评估每一路音频的音量大小，并每隔一段时间抛出该事件回调，您可以根据音量大小在 UI
   * 上做出相应的提示，比如`波形图`或`音量槽`。 要完成这个功能， 您需要先调用 {@link
   * enableAudioVolumeEvaluation} 开启这个能力并设定事件抛出的时间间隔。
   ...
   * @note userVolumes 为一个数组，对于数组中的每一个元素，当 userId
   * 为空时表示本地麦克风采集的音量大小，当 userId 不为空时代表远端用户的音量大小。
   */
 ...
 *
 * 在 Mac 或 Windows
 * 这样的桌面操作系统上，用户可以在设置中心找到声音相关的设置面板，并设置麦克风的采集音量大小。
 ...
 * @note 您需要调用 {@link enableAudioVolumeEvaluation} 接口并设定 interval 大于 0
 * 开启次事件回调，设定 interval 等于 0 关闭此事件回调。
 */
#if TARGET_PLATFORM_DESKTOP
 ...
 *
 * 在 Mac 或 Windows
 * 这样的桌面操作系统上，用户可以在设置中心找到声音相关的设置面板，并设置系统的播放音量大小。
 ...
 *
 * 在 Mac 系统上，您可以通过调用 {@link startSystemAudioLoopback} 为当前系统安装一个音频驱动，并让
 * SDK 通过该音频驱动捕获当前 Mac 系统播放出的声音。
 * 当用于播片教学或音乐直播中，比如老师端可以使用此功能，让 SDK
 * 能够采集老师所播放的电影中的声音，使同房间的学生端也能听到电影中的声音。 SDK
 * 会将统声音采集是否被成功开启的结果，通过本事件回调抛出，需要您关注参数中的错误码。
 ...
 * 6.9 测试麦克风时的音量回调
 *
 * 当您调用 {@link startMicDeviceTest} 测试麦克风是否正常工作时，SDK 会不断地抛出此回调，参数中的
 * volume 代表当前麦克风采集到的音量大小。 如果在测试期间 volume
 * 出现了大小波动的情况，说明麦克风状态健康；如果 volume 的数值始终是
 * 0，说明麦克风的状态异常，需要提示用户进行更换。
 ...
 * 参数中的 volume 代表的是 SDK
 * 提交给系统扬声器去播放的声音的音量值大小，如果该数值持续变化，但用户反馈听不到声音，则说明扬声器状态异常。
 ...
 */
   ...
   * 7.1 收到自定义消息的事件回调
   *
   * 当房间中的某个用户使用 {@link sendCustomCmdMsg} 发送自定义 UDP 消息时，房间中的其它用户可以通过
   * onRecvCustomCmdMsg 事件回调接收到该条消息。
   * @param userId 用户标识。
   * @param cmdID 命令 ID。
   * @param seq   消息序号。
   * @param message 消息数据。
   */
  virtual void onRecvCustomCmdMsg(const char* userId,
                                  int32_t cmdID,
                                  uint32_t seq,
   ...
   * 7.2 自定义消息丢失的事件回调
   *
   * 当您使用 {@link sendCustomCmdMsg} 发送自定义 UDP
   * 消息时，即使设置了可靠传输（reliable），也无法确保100@%不丢失，只是丢消息概率极低，能满足常规可靠性要求。
   * 在发送端设置了可靠运输（reliable）后，SDK
   * 都会通过此回调通知过去时间段内（通常为5s）传输途中丢失的自定义消息数量统计信息。
   * @param userId 用户标识。
   * @param cmdID 命令 ID。
   * @param errCode 错误码。
   ...
   */
  virtual void onMissCustomCmdMsg(const char* userId,
                                  int32_t cmdID,
                                  int32_t errCode,
                                  int32_t missed) {}
   ...
   * 7.3 收到 SEI 消息的回调
   *
   * 当房间中的某个用户使用 {@link sendSEIMsg} 借助视频数据帧发送 SEI
   * 消息时，房间中的其它用户可以通过 onRecvSEIMsg 事件回调接收到该条消息。
   * @param userId   用户标识。
   * @param message  数据。
   */
   ...
   *
   * 当您调用 {@link startPublishing} 开始向腾讯云直播 CDN 上发布音视频流时，SDK
   * 会立刻将这一指令同步给云端服务器。 随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   ...
   *
   * 当您调用 {@link stopPublishing} 停止向腾讯云直播 CDN 上发布音视频流时，SDK
   * 会立刻将这一指令同步给云端服务器。 随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param err 0表示成功，其余值表示失败。
   * @param errMsg 具体错误原因。
   ...
   *
   * 当您调用 {@link startPublishCDNStream} 开始向非腾讯云直播 CDN 上发布音视频流时，SDK
   * 会立刻将这一指令同步给云端服务器。随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param err 0表示成功，其余值表示失败。
   * @param errMsg 具体错误原因。
   * @note 当您收到成功的事件回调时，只是说明您的发布指令已经同步到腾讯云后台服务器，但如果目标 CDN
   * 厂商的服务器不接收该条视频流，依然可能导致发布失败。
   */
   ...
   *
   * 当您调用 {@link stopPublishCDNStream} 开始向非腾讯云直播 CDN 上发布音视频流时，SDK
   * 会立刻将这一指令同步给云端服务器。随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param err 0表示成功，其余值表示失败。
   * @param errMsg 具体错误原因。
   ...
   *
   * 当您调用 {@link setMixTranscodingConfig} 调整云端混流的排版布局和转码参数时，SDK
   * 会立刻将这一调整指令同步给云端服务器。随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   ...
   * 8.6 开始发布媒体流的事件回调
   *
   * 当您调用 {@link startPublishMediaStream} 开始向 TRTC 后台服务发布媒体流时，SDK
   * 会立刻将这一指令同步给云端服务器，随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param taskId 当请求成功时，TRTC 后台会在回调中提供给您这项任务的 taskId，后续您可以通过该
   * taskId 结合 {@link updatePublishMediaStream} 和 {@link stopPublishMediaStream} 进行更新和停止。
   * @param code 回调结果，0 表示成功，其余值表示失败。
   * @param message 具体回调信息。
   * @param extraInfo 扩展信息字段，个别错误码可能会带额外的信息帮助定位问题。
   */
  virtual void onStartPublishMediaStream(const char* taskId,
                                         int code,
                                         const char* message,
                                         void* extraInfo) {}
   ...
   * 8.7 更新媒体流的事件回调
   *
   * 当您调用媒体流发布接口 ({@link updatePublishMediaStream}) 开始向 TRTC 后台服务更新媒体流时，SDK
   * 会立刻将这一指令同步给云端服务器，随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param taskId 您调用媒体流发布接口 ({@link updatePublishMediaStream}) 时传入的
   * taskId，会通过此回调再带回给您，用于标识该回调属于哪一次更新请求。
   * @param code 回调结果，0 表示成功，其余值表示失败。
   * @param message 具体回调信息。
   * @param extraInfo 扩展信息字段，个别错误码可能会带额外的信息帮助定位问题。
   */
  virtual void onUpdatePublishMediaStream(const char* taskId,
                                          int code,
                                          const char* message,
                                          void* extraInfo) {}
   ...
   * 8.8 停止媒体流的事件回调
   *
   * 当您调用停止发布媒体流 ({@link stopPublishMediaStream}) 开始向 TRTC 后台服务停止媒体流时，SDK
   * 会立刻将这一指令同步给云端服务器，随后 SDK
   * 会收到来自云端服务器的处理结果，并将指令的执行结果通过本事件回调通知给您。
   * @param taskId 您调用停止发布媒体流 ({@link stopPublishMediaStream}) 时传入的
   * taskId，会通过此回调再带回给您，用于标识该回调属于哪一次停止请求。
   * @param code 回调结果，0 表示成功，其余值表示失败。
   * @param message 具体回调信息。
   * @param extraInfo 扩展信息字段，个别错误码可能会带额外的信息帮助定位问题。
   */
  virtual void onStopPublishMediaStream(const char* taskId,
                                        int code,
                                        const char* message,
                                        void* extraInfo) {}
   ...
   *
   * 当您调用 {@link startPublishMediaStream} 开始向 TRTC 后台服务发布媒体流时，SDK
   * 会立刻将这一指令同步给云端服务。 若您在目标推流配置 ({@link TRTCPublishTarget})
   * 设置了向腾讯或者第三方 CDN 上发布音视频流的 URL 配置，则具体 RTMP 或者 RTMPS
   * 推流状态将通过此回调同步给您。
   * @param cdnUrl 您调用 {@link startPublishMediaStream} 时通过目标推流配置 ({@link
   * TRTCPublishTarget}) 传入的 url，在推流状态变更时，会通过此回调同步给您。
   * @param status 推流状态。
   * - 0：推流未开始或者已结束。在您调用 {@link stopPublishMediaStream} 时会返回该状态。
   * - 1：正在连接 TRTC 服务器和 CDN 服务器。若无法立刻成功，TRTC
   * 后台服务会多次重试尝试推流，并返回该状态（5s回调一次）。如成功进行推流，则进入状态
   * 2；如服务器出错或 60 秒内未成功推流，则进入状态 4。
   * - 2：CDN 推流正在进行。在成功推流后，会返回该状态。
   * - 3：TRTC 服务器和 CDN 服务器推流中断，正在恢复。当 CDN 出现异常，或推流短暂中断时，TRTC
   * 后台服务会自动尝试恢复推流，并返回该状态（5s回调一次）。如成功恢复推流，则进入状态
   * 2；如服务器出错或 60 秒内未成功恢复，则进入状态 4。
   * - 4：TRTC 服务器和 CDN 服务器推流中断，且恢复或连接超时。即此时推流失败，你可以再次调用 {@link
   * updatePublishMediaStream} 尝试推流。
   * - 5：正在断开 TRTC 服务器和 CDN 服务器。在您调用 {@link stopPublishMediaStream} 时，TRTC
   * 后台服务会依次同步状态 5 和状态 0。
   * @param code 推流结果，0 表示成功，其余值表示出错。
   * @param message 具体推流信息。
   * @param extraInfo 扩展信息字段，个别错误码可能会带额外的信息帮助定位问题。
   */
  virtual void onCdnStreamStateChanged(const char* cdnUrl,
                                       int status,
                                       int code,
                                       const char* msg,
                                       void* extraInfo) {}
 ...
 *
 * 当屏幕分享的目标窗口被遮挡无法正常捕获时，SDK 会抛出此事件回调，你可以在捕获到该事件回调后，通过
 * UI 上的一些变化来提示用户移开遮盖窗口。
 */
#ifdef _WIN32
   ...
   *
   * 当您调用 {@link startLocalRecording} 启动本地媒体录制任务时，SDK
   * 会抛出该事件回调，用于通知您录制任务是否已经顺利启动。
   * @param errCode 状态码。
   *               - 0：录制任务启动成功。
   *               - -1：内部错误导致录制任务启动失败。
   ...
   *
   * 当您调用 {@link stopLocalRecording} 停止本地媒体录制任务时，SDK
   * 会抛出该事件回调，用于通知您录制任务的最终结果。
   * @param errCode 状态码。
   *               -  0：结束录制任务成功。
   *               - -1：录制失败。
   *               - -2：切换分辨率或横竖屏导致录制结束。
   ...
   * @note 全平台 C++ 接口和 Java 接口在参数上是不一样的，C++ 接口用 7 个参数描述一个截图画面，Java
   * 接口只用一个 Bitmap 描述一个截图画面。
   */
 ...
 */
#ifdef _WIN32
  trtc_attribute_deprecated virtual void onPlayBGMProgress(uint32_t progressMS,
                                                           uint32_t durationMS) {}
#endif
 ...
 */
   ...
   */
  trtc_attribute_deprecated virtual void onSpeedTest(const TRTCSpeedTestResult& currentResult,
...
class ITRTCVideoRenderCallback {
 public:
   ...
   * 自定义视频渲染回调
   *
   * 当您设置了本地或者远端的视频自定义渲染回调之后，SDK
   * 就会将原本要交给渲染控件进行渲染的视频帧通过此回调接口抛送给您，以便于您进行自定义渲染。
   * @param frame  待渲染的视频帧信息。
   * @param userId 视频源的
   * userId，如果是本地视频回调（setLocalVideoRenderDelegate），该参数可以忽略。
   * @param streamType
...
class ITRTCVideoFrameCallback {
 public:
   ...
   * 用于对接第三方美颜组件的视频处理回调
   *
   * 如果您选购了第三方美颜组件，就需要在 TRTCCloud 中设置第三方美颜回调，之后 TRTC
   * 就会将原本要进行预处理的视频帧通过此回调接口抛送给您。 之后您就可以将 TRTC
   * 抛出的视频帧交给第三方美颜组件进行图像处理，由于抛出的数据是可读且可写的，因此第三方美颜的处理结果也可以同步给
   * TRTC 进行后续的编码和发送。 情况一：美颜组件自身会产生新的纹理。
   * 如果您使用的美颜组件会在处理图像的过程中产生一帧全新的纹理（用于承载处理后的图像），那请您在回调函数中将
   * dstFrame.textureId 设置为新纹理的 ID： <pre> int onProcessVideoFrame(TRTCVideoFrame * srcFrame,
   * TRTCVideoFrame *dstFrame) { dstFrame->textureId =
   * mFURenderer.onDrawFrameSingleInput(srcFrame->textureId); return 0;
   * }
   * </pre>
   *
   * 情况二：美颜组件需要您提供目标纹理。
...
class ITRTCAudioFrameCallback {
 public:
   ...
   * 内部会把刚采集到并经过前处理(ANS、AEC、AGC）之后的数据，以 PCM 格式的形式通过本接口回调给您。
   * - 此接口回调出的音频时间帧长固定为0.02s，格式为 PCM 格式。
   * - 由时间帧长转化为字节帧长的公式为 `采样率 × 时间帧长 × 声道数 × 采样点位宽`。
   * - 以 TRTC 默认的音频录制格式48000采样率、单声道、16采样点位宽为例，字节帧长为 `48000 × 0.02s ×
   * 1 × 16bit = 15360bit = 1920字节`。
   * @param frame PCM 格式的音频数据帧。
   * @note
   * 1. 请不要在此回调函数中做任何耗时操作，由于 SDK 每隔 20ms
   * 就要处理一帧音频数据，如果您的处理时间超过 20ms，就会导致声音异常。
   * 2.
   * 此接口回调出的音频数据是可读写的，也就是说您可以在回调函数中同步修改音频数据，但请保证处理耗时。
   * 3.
   * 此接口回调出的音频数据已经经过了前处理(ANS、AEC、AGC），但**不包含**背景音、音效、混响等前处理效果，延迟较低。
   */
   ...
   *
   * 当您设置完音频数据自定义回调之后，SDK 内部会把刚采集到并经过前处理、音效处理和混 BGM
   * 之后的数据，在最终进行网络编码之前，以 PCM 格式的形式通过本接口回调给您。
   * - 此接口回调出的音频时间帧长固定为0.02s，格式为 PCM 格式。
   * - 由时间帧长转化为字节帧长的公式为`采样率 × 时间帧长 × 声道数 × 采样点位宽`。
   * - 以 TRTC 默认的音频录制格式48000采样率、单声道、16采样点位宽为例，字节帧长为`48000 × 0.02s × 1
   * × 16bit = 15360bit = 1920字节`。 特殊说明： 您可以通过设置接口中的 `TRTCAudioFrame.extraData`
   * 字段，达到传输信令的目的。由于音频帧头部的数据块不能太大，建议您写入 `extraData`
   * 时，尽量将信令控制在几个字节的大小，如果超过 100 个字节，写入的数据不会被发送。
   * 房间内其他用户可以通过 {@link ITRTCAudioFrameCallback} 中的 `onRemoteUserAudioFrame` 中的
   * `TRTCAudioFrame.extraData` 字段回调接收数据。
   * @param frame PCM 格式的音频数据帧。
   * @note
   * 1. 请不要在此回调函数中做任何耗时操作，由于 SDK 每隔 20ms
   * 就要处理一帧音频数据，如果您的处理时间超过 20ms，就会导致声音异常。
   * 2.
   * 此接口回调出的音频数据是可读写的，也就是说您可以在回调函数中同步修改音频数据，但请保证处理耗时。
   * 3. 此接口回调出的数据已经经过了前处理(ANS、AEC、AGC）、音效和混 BGM 处理，声音的延迟相比于
   * {@link onCapturedAudioFrame} 要高一些。
   */
   ...
   * 混音前的每一路远程用户的音频数据
   *
   * 当您设置完音频数据自定义回调之后，SDK 内部会把远端的每一路原始数据，在最终混音之前，以 PCM
   * 格式的形式通过本接口回调给您。
   * - 此接口回调出的音频时间帧长固定为0.02s，格式为 PCM 格式。
   * - 由时间帧长转化为字节帧长的公式为`采样率 × 时间帧长 × 声道数 × 采样点位宽`。
   * - 以 TRTC 默认的音频录制格式48000采样率、单声道、16采样点位宽为例，字节帧长为`48000 × 0.02s × 1
   * × 16bit = 15360bit = 1920字节`。
   * @param frame PCM 格式的音频数据帧。
   * @param userId 用户标识。
...
class ITRTCLogCallback {
 public:
   ...
   * 本地 LOG 的打印回调
   *
   * 如果您希望捕获 SDK 的本地日志打印行为，可以通过设置日志回调，让 SDK
   * 将要打印的日志都通过本回调接口抛送给您。
   * @param log 日志内容。
